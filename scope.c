...
void* scope_last_entity_from_sccope_stop_at(struct scope* scope, struct scope* stop_scope){
    ...
    if(!parent){
        return scope_last_entity_from_sccope_stop_at(parent,stop_scope);
    }
...
void* scope_last_entity_stop_at(struct compile_process* process, struct scope* stop_scope){
    return scope_last_entity_from_sccope_stop_at(process->scope.current,stop_scope);
}

void* scope_last_entity(struct compile_process* process){
    return scope_last_entity_from_sccope_stop_at(process->scope.current,NULL);
}
